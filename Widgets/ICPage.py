...
class ICWidget(Base, Form):
    ...
    def accelerationBtnPressed(self):
        ...
        self.acceleration_timer.timeout.connect(lambda: self.updateSpeedAndEngineRpm("Accelerate"))
    ...
    def accelerationBtnReleased(self):
        if self.Speed_slider.value() <= 0:
            ...
            self.acceleration_timer.timeout.connect(lambda: self.updateSpeedAndEngineRpm("Decelerate"))
            self.acceleration_timer.start(100)
    ...
    def updateSpeedAndEngineRpm(self, action, acceleration=(60/5)):
        ...
        self.current_speed = AccelerationFns.calculate_speed(duration, acceleration)
        self.current_rpm = AccelerationFns.calculate_engine_rpm(self.current_speed)
...
# create a class ICScript and inherit from the ICWidget class
class ICScript(ICWidget):
    def start_script(self):
        ...
        for rate in rates:
            ...
            ICWidget.acceleration_timer.timeout.connect(lambda: ICWidget.updateSpeedAndEngineRpm("Accelerate"), rate)
            ICWidget.acceleration_timer.start(100)
            time.sleep(5)
            ICWidget.accelerationBtnReleased()
            ICWidget.acceleration_timer.timeout.connect(lambda: ICWidget.updateSpeedAndEngineRpm("Decelerate"), rate)
            ICWidget.acceleration_timer.start(100)
            time.sleep(5)
